	...
	return function(req,res,next){
		...
		fs_stat(realPath).then(function(stat){
			...
			if (type === "less") {
				...
				if(isFile && Con.compressJS){
					var minifyjs = UglifyJS.minify(realPath,UglifyJSConfig);
					return resEnd(minifyjs.code);
				}else if(isCancatFile){
					var minifyjs = UglifyJS.minify(realPaths,UglifyJSConfig);
					return resEnd(minifyjs.code);
				}
			}
